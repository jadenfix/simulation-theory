...
def test_regret_uses_same_experiment_cost_accounting_for_controller_and_oracle():
    ...
    assert result.valid
    # Both the shared controller and model-informed singleton oracle are forced
    # to pay the same acquisition cost, so no artificial regret is created by
    # charging only one side.
