...
def test_exact_bayes_update_and_markov_prediction_are_rational():
    ...
    assert tuple(branch.probability for branch in initial) == (
        Fraction(1, 2),
        Fraction(1, 2),
    )
...
def test_k3_no_partial_and_full_observation_have_strict_expected_cost_chain():
    ...
    none = exact_bayesian_coding_game(
        graph,
        model,
        no_information_kernel(3),
        prior,
        2,
    )
    partial = exact_bayesian_coding_game(
        graph,
        model,
        deterministic_partition_kernel((0, 1, 1)),
        prior,
        2,
    )
    full = exact_bayesian_coding_game(
        graph,
        model,
        full_information_kernel(3),
        prior,
        2,
    )
...
def test_stochastic_garbling_has_exact_intermediate_information_value():
    ...
    reveal_or_unknown = garble_observation_kernel(
        full,
        (
    ...
    noisy_comparison = exact_blackwell_observation_value(
        graph,
        model,
        prior,
        1,
    ...
    erase_everything = garble_observation_kernel(
        reveal_or_unknown.coarser,
        ((1,), (1,), (1,), (1,)),
    )
    no_information_comparison = exact_blackwell_observation_value(
        graph,
        model,
        prior,
        1,
...
def test_switching_penalty_is_part_of_the_belief_state_control_problem():
    ...
    model = bayesian_law_model(_point_masses(3), _identity_transition(3))
    prior = (Fraction(1, 3),) * 3
    kernel = full_information_kernel(3)

    result = exact_bayesian_coding_game(
        graph,
        model,
        kernel,
        prior,
        3,
        switching_penalty=Fraction(5),
    )
    assert result.valid
    # The hidden state is static and fully revealed before the first action, so
    # no later switch is useful or required despite the large declared penalty.
    assert result.initial_value == 3


def test_deterministic_partition_is_a_garbling_of_full_information():
    full = full_information_kernel(3)
    partition = garble_observation_kernel(
        full,
        (
            (1, 0),
            (0, 1),
