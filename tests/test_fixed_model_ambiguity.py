    ...
    full_signal = fixed_model_family(
        (
            fixed_model_scenario(
                "A",
                model_a,
                observation_kernel(((1, 0),)),
                (1,),
            ),
            fixed_model_scenario(
                "B",
                model_b,
                observation_kernel(((0, 1),)),
                (1,),
            ),
        )
    )
...
def test_identical_models_have_zero_model_consistency_gap():
    ...
    family = fixed_model_family(
        (
            fixed_model_scenario("copy-1", model, no_information_kernel(1), (1,)),
            fixed_model_scenario("copy-2", model, no_information_kernel(1), (1,)),
        )
    )
...
def test_model_family_validation_and_frontier_caps_are_explicit():
    ...
    with pytest.raises(ValueError):
        fixed_model_family(
            (
                fixed_model_scenario("same", model, no_information_kernel(1), (1,)),
                fixed_model_scenario("same", model, no_information_kernel(1), (1,)),
            )
        )
    ...
    family = fixed_model_family(
        (
            fixed_model_scenario("A", model, no_information_kernel(1), (1,)),
            fixed_model_scenario("B", model, no_information_kernel(1), (1,)),
        )
    )
    with pytest.raises(ValueError):
        exact_fixed_model_ambiguity_game(
