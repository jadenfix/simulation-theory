...
def test_strict_three_level_information_refinement_chain():
    ...
    coarse_to_middle = exact_partition_refinement_value(graph, identity, coarse, middle, (0, 1, 2), 2)
    middle_to_fine = exact_partition_refinement_value(graph, identity, middle, fine, (0, 1, 2), 2)
...
def test_vector_frontier_prevents_hidden_state_reselection_across_time():
    ...
    laws = _point_masses(3)
    identity = finite_law_transition_model(laws, ((0,), (1,), (2,)))
    certificate = exact_coarsened_observation_game(graph, identity, no_observation_partition(3), (0, 1, 2), 3)
    assert certificate.valid
...
def test_initial_observation_can_have_value_before_the_first_code_choice():
    ...
    laws = _point_masses(3)
    identity = finite_law_transition_model(laws, ((0,), (1,), (2,)))
    none = exact_coarsened_observation_game(graph, identity, no_observation_partition(3), (0, 1, 2), 1)
    full = exact_coarsened_observation_game(graph, identity, full_observation_partition(3), (0, 1, 2), 1)
...
def test_switching_penalty_is_replayed_on_information_set_policy_paths():
    ...
    laws = _point_masses(3)
    identity = finite_law_transition_model(laws, ((0,), (1,), (2,)))
    result = exact_coarsened_observation_game(graph, identity, no_observation_partition(3), (0, 1, 2), 2, switching_penalty=Fraction(2))
    assert result.valid
...
def test_directed_transition_paths_remain_actual_state_consistent():
    graph = _complete_graph(3)
    model = finite_law_transition_model(_point_masses(3), ((0, 1), (1, 2), (2,)))
    result = exact_coarsened_observation_game(graph, model, observation_partition((0, 0, 1)), (0,), 3)
    assert result.valid
    assert all(right in model.successors[left] for left, right in zip(result.adversarial_path, result.adversarial_path[1:]))
    assert all(mask & (1 << state) for mask, state in zip(result.information_path, result.adversarial_path))
...
def test_invalid_partitions_initial_sets_and_caps_are_rejected():
    ...
    with pytest.raises(ValueError):
        observation_partition(())
    with pytest.raises(ValueError):
        exact_coarsened_observation_game(graph, model, observation_partition((0, 0)), (0,), 2)
    with pytest.raises(ValueError):
        exact_coarsened_observation_game(graph, model, no_observation_partition(3), (), 2)
    with pytest.raises(ValueError):
        exact_coarsened_observation_game(graph, model, no_observation_partition(3), (0, 1, 2), 3, max_frontier_entries=1)
