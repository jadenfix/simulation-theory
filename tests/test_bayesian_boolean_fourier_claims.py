...
def test_bayesian_boolean_fourier_claim_manifest_is_valid_and_evidence_bound():
    path = Path(__file__).parents[1] / "claims" / "bayesian-boolean-fourier-claims.json"
    ...
    assert {claim["id"] for claim in manifest["claims"]} == {
        "ST-T210", "ST-T211", "ST-T212", "ST-T213", "ST-T214", "ST-M60", "ST-F36"
    }
