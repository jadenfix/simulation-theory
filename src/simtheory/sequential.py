...
def bernoulli_log_likelihood_ratio(
    successes: int,
    trials: int,
...
def mixture_log_e_value(
    successes: int,
    trials: int,
    null_probability: float,
    alternatives: Sequence[float],
...
def exact_expected_e_value(
    trials: int,
    true_probability: float,
    null_probability: float,
    alternative_probability: float,
) -> float:
    """Exact fixed-time expectation under an IID Bernoulli true law."""
    if trials < 0:
...
def exact_anytime_rejection_probability(
    horizon: int,
    true_probability: float,
    null_probability: float,
    alternative_probability: float,
    ...
    """Dynamic-program the probability of crossing 1/alpha by a horizon.

    States are ``(trials, successes)`` because the fixed-alternative Bernoulli
    likelihood ratio depends on the path only through those sufficient
    statistics. This is exact up to floating-point arithmetic and costs O(T^2),
    not O(2^T).
    """
    ...
    for trials in range(horizon + 1):
        ...
        for successes, probability in frontier.items():
            log_e = bernoulli_log_likelihood_ratio(
                successes,
                trials,
                null_probability,
                alternative_probability,
            ...
            if trials == horizon:
                continue
            next_frontier[successes + 1] = (
                next_frontier.get(successes + 1, 0.0) + probability * true_p
            )
            next_frontier[successes] = (
                next_frontier.get(successes, 0.0) + probability * (1.0 - true_p)
            )
        frontier = next_frontier
    return rejected_probability


def fixed_alternative_is_composite_null_eprocess(
    true_probability: float,
    null_boundary: float,
    alternative_probability: float,
) -> bool:
    """Check the one-sided Bernoulli supermartingale direction.

    If the alternative exceeds the boundary, the LR is an e-process for every
    IID Bernoulli null with p <= boundary. If the alternative is below the
    boundary, it is valid for every p >= boundary.
    """
    true_p = _open_probability(true_probability, "true_probability")
...
def exact_expected_mixture_e_value(
    trials: int,
    true_probability: float,
    null_probability: float,
    alternatives: Sequence[float],
...
def exact_anytime_mixture_rejection_probability(
    horizon: int,
    true_probability: float,
    null_probability: float,
    alternatives: Sequence[float],
    ...
    """Exact finite-horizon crossing probability for a mixture e-process."""
    if horizon < 0 or horizon > 10_000:
        raise ValueError("horizon must lie in [0,10000]")
    true_p = _open_probability(true_probability, "true_probability")
    alpha = _open_probability(alpha, "alpha")
